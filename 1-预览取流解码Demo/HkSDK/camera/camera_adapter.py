...
class CameraAdapter(BaseAdapter):
    ...
    def start_preview(self,hPlayWnd=0,lChannel=1,dwStreamType=0,dwLinkMode=0,bBlocked=1,cbFunc=None):
        ...
        # pUser = id(self)
        funcRealDataCallBack_V30 = HCNetSDK.REALDATACALLBACK(self.RealDataCallBack_V30)
        self.lRealPlayHandle = self.HCNetSDK_obj.NET_DVR_RealPlay_V40(self.camera_id, ctypes.byref(preview_info), cbFunc, None)
    ...
    def callback_real_data(self, lRealPlayHandle, cbFunc, dwUser):
        err = self.HCNetSDK_obj.NET_DVR_SetRealDataCallBack(lRealPlayHandle, cbFunc, dwUser)
        if not err:
            print("err")

    def callback_standard_data(self, lRealPlayHandle, cbFunc, dwUser):
        err = self.HCNetSDK_obj.NET_DVR_SetStandardDataCallBack(lRealPlayHandle, cbFunc, dwUser)
        if not err:
            print("err")
    ...
    def RealDataCallBack_V30(lPlayHandle,dwDataType, pBuffer, dwBufSize, pUser):
        global cnt
        # self_instance = ctypes.cast(pUser, ctypes.py_object).value
        self_instance = ctypes.py_object.from_address(pUser).value
        # 码流回调函数
        print("回调回调")
        if dwDataType == HCNetSDK.NET_DVR_SYSHEAD:
            # 设置流播放模式
            
            self_instance.PlayCtrl_obj.PlayM4_SetStreamOpenMode(self_instance.PlayCtrl_Port, 0)
            # 打开码流，送入40字节系统头数据
            if self_instance.PlayCtrl_obj.PlayM4_OpenStream(self_instance.PlayCtrl_Port, pBuffer, dwBufSize, 1024*1024):
                # 设置解码回调，可以返回解码后YUV视频数据
                # global FuncDecCB
                # callback_with_self_t = lambda nPort, pBuf, nSize, pFrameInfo, nUser, nReserved2: CameraAdapter.DecCBFun(nPort, pBuf, nSize, pFrameInfo, nUser, nReserved2)
                self_instance.FuncDecCB = PlayCtrl.DECCBFUNWIN(self_instance.DecCBFun)
                self_instance.PlayCtrl_obj.PlayM4_SetDecCallBackExMend(self_instance.PlayCtrl_Port, self_instance.FuncDecCB, None, 0, None)
                # 开始解码播放
                if self_instance.PlayCtrl_obj.PlayM4_Play(self_instance.PlayCtrl_Port, self_instance.CanvasHandle):
                    print(u'播放库播放成功')
                else:
                    print(u'播放库播放失败')
            else:
            ...
            cnt=cnt+1
            self_instance.PlayCtrl_obj.PlayM4_InputData(self_instance.PlayCtrl_Port, pBuffer, dwBufSize)
        else:
            print (u'其他数据,长度:', dwBufSize)
    
    @staticmethod
    def DecCBFun(nPort, pBuf, nSize, pFrameInfo, nUser, nReserved2):          # 解码回调函数
